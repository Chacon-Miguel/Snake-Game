...
(* Exercise 01:
   ...
   reverse order),
   2. add the location of the snake's current head to the snake's body, and
   3. determine the new location of the snake's head.
...
(* Exercise 04a:
   ...
   [List.for_all] takes a list and a predicate, "f", and returns true if "f" applied to
   all of its arguments is true, and false otherwise.

   [List.exists] takes a list and a predicate, "f", and returns true if "f" applied to at
   least one of its arguments is true, and false otherwise.
   ...
   At the call site, it's ambiguous which int is the height and which int is the width, so
   it's very easy to supply arguments in the wrong order.
